...
@Component({
    selector: 'addmaterialsumco-widget',
    templateUrl: './addMaterialSumco.widget.html'
})
export class AddMaterialSumcoWidget {
    ...
    existe(idProducto: number, esSumco: Number) {
        //INICIALIZAR VALORES DEL MATERIAL SELECCIONADO
    ...
    open(idProducto: number, esSumco: number) {
        if (idProducto == 0) {
            this.nuevo(esSumco);
        } else {
            this.existe(idProducto, esSumco);
        }
        let docModal = document.getElementById('modalAddMaterialSumco');
